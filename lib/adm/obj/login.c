...
void idle_email(string str)
{
    ...
    else
    {
        ...
        if (file_exists("/adm/etc/new_install"))
        {
            ...
            catch (cp("/areas/std/workroom.c", user_path(username)));
            write_file(
...
void enterWorld(string str){
    ...
    else if(devp(user)) body->move(user_path(query_privs(user))+"workroom.c");
    else body->move("/areas/trepi/docks/room/dock9.c");
    if (!environment(body)) body->move("/areas/trepi/docks/room/dock9.c");
    body->enter_world();
